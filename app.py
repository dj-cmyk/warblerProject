...
def profile():
    ...
    get_user_by_id = User.query.get_or_404(g.user.id)
    form = UserEditForm(obj=get_user_by_id)
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            g.user.image_url = request.form.get("image_url", g.user.image_url)
            g.user.header_image_url = request.form.get("header_image_url", g.user.header_image_url)
...
def like_warble(message_id):
    ...
    if g.user.id == message.user_id:
        flash("Can't Like Your Own Warbles", "warning")
        return redirect("/")
    else:
        like = Likes(user_id=g.user.id, message_id=message_id)
        db.session.add(like)
        db.session.commit()
        # on dom -> change icon to star / link to unlike same message
        # also need to make sure that you can't attempt to like same message twice
        return redirect('/')
